#!/usr/bin/env bash
# Extensión map_space_saturation_v1: 800 → 1000 → 1200 + análisis de saturación.
...
#   bash scenarios/setup/run_saturation_extension_1000_1200.sh
#   bash scenarios/setup/run_saturation_extension_1000_1200.sh --from-phase=E
#   bash scenarios/setup/run_saturation_extension_1000_1200.sh --skip-synth-rebuild
#
# Salida de reproducibilidad:
#   scenarios/analysis/data/map_space_saturation_run_manifest.json
...
MANIFEST_JSON="$REPO_ROOT/scenarios/analysis/data/map_space_saturation_run_manifest.json"
LOG_DIR="$REPO_ROOT/scenarios/analysis/logs"
RUN_LOG="$LOG_DIR/map_space_saturation_extension.log"

SEED="${SEED:-42}"
TARGET_TOTAL="${TARGET_TOTAL:-1200}"
FROM_PHASE="${FROM_PHASE:-A}"
SKIP_SYNTH_REBUILD=false
MAX_OSM_ROUNDS="${MAX_OSM_ROUNDS:-60}"
OSM_BATCH="${OSM_BATCH:-25}"

# phase status (ok|fail|skip)
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'EOF'
Uso: run_saturation_extension_1000_1200.sh [opciones]
...
Opciones:
  ...
  --target-total=1200      Total de candidatos planificados
...
log "=== Saturación extensión target_total=$TARGET_TOTAL seed=$SEED from_phase=$FROM_PHASE ==="
log "Log: $RUN_LOG"
...
# --- C: OSM loop (tolerante: una ronda fallida no detiene el bucle) ---
...
# --- E: análisis (siempre intentar aunque build haya fallado parcialmente) ---
...
doc = {
    "pipeline": "map_space_saturation_extension_1000_1200",
    ...
    "saturation_decision": {
        ...
        for k in (
            ...
            "extension_confirmed", "max_batch_evaluated", "max_generated",
