...
def generate_hub_and_spoke(params: dict[str, Any], rng: random.Random) -> tuple[list[Edge], dict[str, Any]]:
    ...
    for hub in hubs:
        for i in range(spokes_per_hub):
            angle = 2 * math.pi * i / spokes_per_hub + rng.uniform(-0.1, 0.1)
            end = (hub[0] + spoke_length * math.cos(angle), hub[1] + spoke_length * math.sin(angle))
            all_nodes.append(end)
            _add_edge(edges, hub, end)
            # Ring connections between spoke tips for density
            if i > 0 and rng.random() < 0.35:
                prev_angle = 2 * math.pi * (i - 1) / spokes_per_hub
                prev_end = (
                    hub[0] + spoke_length * 0.85 * math.cos(prev_angle),
                    hub[1] + spoke_length * 0.85 * math.sin(prev_angle),
                )
                _add_edge(edges, end, prev_end)
    if n_hubs > 1:
        for i in range(n_hubs - 1):
            _add_edge(edges, hubs[i], hubs[i + 1])
    xs = [p[0] for p in all_nodes]
    ys = [p[1] for p in all_nodes]
    return edges, {"span_x": max(xs) - min(xs), "span_y": max(ys) - min(ys), "n_nodes": len(all_nodes)}
...
def generate_tree_trails(params: dict[str, Any], rng: random.Random) -> tuple[list[Edge], dict[str, Any]]:
    branch_factor = int(params["branch_factor"])
...
def generate_clustered_communities(params: dict[str, Any], rng: random.Random) -> tuple[list[Edge], dict[str, Any]]:
    ...
    nodes_per_cluster = int(params["nodes_per_cluster"])
    intra_density = float(params["intra_density"])
    inter_gap = float(params["inter_gap_m"])
    inter_bridges = int(params.get("inter_bridge_count", 1))
    edges: list[Edge] = []
    ...
    for i in range(n_clusters):
        ...
        center = (100 + col * inter_gap, 100 + row * inter_gap)
        cluster_centers.append(center)
        local: list[Coord] = []
        for _ in range(nodes_per_cluster):
            local.append(
                (
                    center[0] + rng.uniform(-inter_gap * 0.15, inter_gap * 0.15),
                    center[1] + rng.uniform(-inter_gap * 0.15, inter_gap * 0.15),
                )
            )
        all_nodes.extend(local)
        for a in range(len(local)):
            for b in range(a + 1, len(local)):
                if rng.random() < intra_density:
                    _add_edge(edges, local[a], local[b])
    for i in range(min(inter_bridges, n_clusters - 1)):
...
def generate_conference_event_compact(params: dict[str, Any], rng: random.Random) -> tuple[list[Edge], dict[str, Any]]:
    ...
    hall_count = int(params["hall_count"])
    rooms_per_hall = int(params["rooms_per_hall"])
    corridor_w = float(params["corridor_width_m"])
    room_spacing = float(params["room_spacing_m"])
    inter_gap = float(params["inter_hall_gap_m"])
    margin = 40.0
    edges: list[Edge] = []
    nodes: list[Coord] = []
    x_cursor = margin
    for h in range(hall_count):
        hall_nodes: list[Coord] = []
        cy = margin + corridor_w / 2
        left = (x_cursor, cy)
        right = (x_cursor + rooms_per_hall * room_spacing, cy)
        nodes.extend([left, right])
        hall_nodes.extend([left, right])
        _add_edge(edges, left, right)
        for r in range(rooms_per_hall):
            top = (x_cursor + r * room_spacing + room_spacing / 2, cy + corridor_w / 2 + room_spacing / 3)
            bot = (x_cursor + r * room_spacing + room_spacing / 2, cy - corridor_w / 2 - room_spacing / 3)
            nodes.extend([top, bot])
            _add_edge(edges, top, (x_cursor + r * room_spacing, cy))
            _add_edge(edges, bot, (x_cursor + r * room_spacing, cy))
        if h > 0:
            _add_edge(edges, hall_nodes[0], (x_cursor - inter_gap, cy))
