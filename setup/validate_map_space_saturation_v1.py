#!/usr/bin/env python3
"""
validate_map_space_saturation_v1.py

Validate batch-generated maps from map_space_saturation_v1 before feature
extraction and saturation analysis.
"""
...
MAP_SPACE_ROOT = SCENARIOS_DIR / "map_space_saturation_v1"
MANIFEST_CSV = MAP_SPACE_ROOT / "manifest_maps_all.csv"
VALIDATION_CSV = SCENARIOS_DIR / "analysis" / "data" / "map_space_saturation_validation.csv"
VALIDATION_REPORT = SCENARIOS_DIR / "analysis" / "reports" / "map_space_saturation_validation_report.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate map_space_saturation_v1 maps")
...
def validate_one(
    ...
    *,
    ...
    roads_path = map_root / roads_rel if roads_rel else (wkt_dir / "roads.wkt" if wkt_dir else None)
    preview_path = map_root / preview_rel if preview_rel else (map_root / "previews" / f"{map_id}.png")
    metadata_path = map_root / meta_rel if meta_rel else (wkt_dir / "metadata.json" if wkt_dir else None)
    ...
    if generation_status in ("FAIL_DOWNLOAD", "FAIL_DOWNLOAD_TRANSIENT", "FAIL_DOWNLOAD_PERMANENT"):
        status = "FAIL"
        reasons.append("osm_download_failed")
        notes.append(generation_notes or "OSM download failed")
    ...
    partitioned = bool(bbox_or_params.get("_allow_partitioned", False))
