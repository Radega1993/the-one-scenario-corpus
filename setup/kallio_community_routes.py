"""KallioCommunityCompact community route validation and regeneration."""
...
from family_routes import _nn_tour, generate_community_routes
...
ROUTE_ROLES = {
    "A_community_route.wkt": "primary community axis (residential–commercial)",
    "B_community_route.wkt": "secondary axis (meeting / civic loop)",
}

BORDER_EPS = 2.0
ORIGIN_EPS = 15.0
MIN_DIST_FROM_ORIGIN = 25.0
MAX_STOP_DIST_M = 100.0

ROUTE_FILES = ("A_community_route.wkt", "B_community_route.wkt")
...
def validate_community_route(
    ...
    if other_stops and _route_separation(stops, other_stops) < 60:
        notes.append("low separation from other community route")
...
def _graph_node_routes(rg: RoadGraph, rng: random.Random) -> dict[str, list[tuple[float, float]]]:
    generated = generate_community_routes(rg, rng)
    ...
    for fname, stops in generated.items():
        ...
        if len(refined) >= 4 and max(vertex_distances(rg, refined)) <= 1.0:
            out[fname] = refined[:12]
            continue
        if "A_community" in fname:
        ...
        tour = _nn_tour(rg, seeds)[:12]
        out[fname] = dedupe_consecutive(tour)
    return out


def regenerate_community_routes(
    map_dir: Path,
    rng: random.Random,
    apply: bool,
) -> list[dict]:
    rg, roads_path, _ = load_road_graph("KallioCommunityCompact")
