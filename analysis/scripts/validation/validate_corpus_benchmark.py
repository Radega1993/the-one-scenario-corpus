#!/usr/bin/env python3
"""
Validate corpus_v1 paper benchmark readiness for routing protocol comparison.

Merges manifest + metric CSVs, classifies each scenario, and writes:
  data/corpus_benchmark_validation.csv
  reports/corpus_benchmark_validation.md
"""
...
from lib.report_paths import CORPUS_BENCHMARK_VALIDATION  # noqa: E402
...
def write_report(
    ...
    lines = [
        ...
        "## Executive summary",
        "",
        f"- **Corpus:** `corpus_v1` — **{completeness['settings_count']}** scenarios (45 bases × 12 TP)",
    ...
    lines.extend([
        ...
        "| Check | Result |",
        "|-------|--------|",
        f"| `.settings` in corpus_v1 | {completeness['settings_count']} |",
...
def main() -> int:
    ...
    ap.add_argument("--manifest", type=str, default=str(COMBINED_MANIFEST_CSV))
    ap.add_argument("--data-dir", type=str, default=str(DATA_DIR))
    ap.add_argument("--output-csv", type=str, default=str(DATA_DIR / "corpus_benchmark_validation.csv"))
    ap.add_argument("--output-report", type=str, default=str(CORPUS_BENCHMARK_VALIDATION))
    ap.add_argument(
        "--refresh-diagnosis",
        action="store_true",
