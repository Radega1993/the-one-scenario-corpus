...
6. **`--phase ablation`**: Compare diversity metrics for 17 vs 23 vs 46 features (max |r|, mean |r|, pairs >=0.7, Silhouette). `reports/pipeline/ablation_report.txt`, `data/ablation_metrics.csv`.
...
## Useful simulation time (corpus_v1)
...
## Corpus audit & v2 revision planning (corpus_v1)

Reproducible pipeline that audits `.settings`, cross-checks simulation metrics, assigns problem flags, and produces a **prioritized revision plan for corpus_v1 in-place** (legacy pre-reorg runs used 720 scenarios; active diversity scope is 540 in `corpus_v1`; no active `corpus_v3/` folder, no automatic `.settings` edits).
...
$PY scenarios/analysis/scripts/validation/analyze_spatial_occupancy.py \
  --manifest scenarios/corpus_v1/manifest.csv --reports-dir reports --skip-heatmaps
...
## Traffic profile validation (corpus_v1)

After regenerating `corpus_v1` or before phase-2 protocol runs:

```bash
python3 scenarios/analysis/scripts/validation/validate_traffic_profiles.py
```

Writes `reports/validation/tp_validation_report.md` and `data/tp_validation_*.csv`. Methodology closure: [../internal/17-benchmark_methodology_closure.md](../internal/17-benchmark_methodology_closure.md).
...
# Parallel run (recommended for corpus_v1)
python3 scenarios/analysis/run_all_scenarios.py --corpus corpus_v1 \
  --extra-settings scenarios/analysis/overlays/routing_contact_reports_overrides.txt \
...
python3 scenarios/analysis/run_all_scenarios.py --corpus corpus_v1 \
  --settings scenarios/corpus_v1/01_urban/U2_SparseSuburb_Manhattan__TP01_Baseline.settings \
  --settings scenarios/corpus_v1/01_urban/U4_CongestionHotspot_Manhattan__TP01_Baseline.settings
...
### Interactive dashboard (corpus_v1)
...
**Useful CSV inputs** (under `scenarios/analysis/data/`): `manifest` from `scenarios/corpus_v1/manifest.csv`; joins include `output_metrics.csv`, `scenario_diagnosis.csv`, `settings_audit.csv`, `spatial_occupancy_metrics.csv`, `message_creation_time_summary.csv`, `message_analysis_window_policy.csv`, `useful_simulation_time_metrics.csv`, `corpus_benchmark_validation.csv`, `traffic_profile_kpi_summary.csv`, `tp_validation_settings.csv`, `indirect_features_diego.csv`.

**Docs:** [`dashboard/README.md`](dashboard/README.md) · readiness: [`reports/paper_gate/dashboard_readiness_report.md`](reports/paper_gate/dashboard_readiness_report.md)
...
Canonical gate: [`reports/paper_freeze_checklist.md`](reports/paper_freeze_checklist.md) · protocol KPIs: [`reports/canonical/protocol_benchmark_kpi_policy.md`](reports/canonical/protocol_benchmark_kpi_policy.md)
