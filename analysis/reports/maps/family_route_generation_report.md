# Family route generation report

Generated: 2026-05-28T17:36:11
Mode: apply
Maps: HelsinkiDowntown, KumpulaCampus, ManhattanMidtownGrid, NuuksioSparseTrails, HelsinkiDisrupted, KallioCommunityCompact, 

Summary CSV: `analysis/data/family_route_generation_summary.csv`

| Map | Route | Stops | Valid | Note |
|-----|-------|-------|-------|------|
| HelsinkiDowntown | A_bus.wkt | 12 | yes | ok |
| HelsinkiDowntown | B_bus.wkt | 11 | yes | ok |
| HelsinkiDowntown | C_bus.wkt | 8 | yes | ok |
| KumpulaCampus | A_campus_shuttle.wkt | 10 | yes | ok |
| ManhattanMidtownGrid | A_vehicle_route.wkt | 14 | yes | ok |
| ManhattanMidtownGrid | B_vehicle_route.wkt | 12 | yes | ok |
| NuuksioSparseTrails | A_ranger_patrol.wkt | 8 | yes | ok |
| HelsinkiDisrupted | A_emergency_route.wkt | 8 | yes | ok |
| HelsinkiDisrupted | B_mule_route.wkt | 8 | yes | ok |
| KallioCommunityCompact | A_community_route.wkt | 8 | yes | ok |
| KallioCommunityCompact | B_community_route.wkt | 8 | yes | ok |
|  | A_control_route.wkt | 10 | yes | ok |