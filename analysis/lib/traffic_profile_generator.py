...
Events1.hosts = {rng_hosts()}
Events1.prefix = M"""
        meta.update({"Events.nrof": "1", "Events1.interval": "180, 300", "Events1.size": "500k, 2M"})
        return block, meta
...
def profile_ttl_minutes(tp_id: str, scenario_base: str = "") -> int:
    ...
    if tp_id == "TP05" and scenario_base in TP05_RELAXED_TTL_BASES:
        return 15
    return {
        "TP01": 7200,
        "TP02": 21600,
...
# Corpus generator CLI removed; see git history / scripts_backup_*.tar.gz
