#!/usr/bin/env python3
"""Extract topology features from validated map_space_revised_v2 (GMS-v1) maps."""
...
REPO_ROOT = _SCENARIOS.parent
SCENARIOS_DIR = _SCENARIOS
DEFAULT_MAP_ROOT = _PACK
DEFAULT_MANIFEST = DEFAULT_MAP_ROOT / "manifest_maps_all.csv"
DEFAULT_VALIDATION = DEFAULT_MAP_ROOT / "data" / "map_space_revised_v2_validation.csv"
DEFAULT_ARCHETYPES = DEFAULT_MAP_ROOT / "data" / "map_archetype_definitions_v1.csv"
DEFAULT_OUTPUT = DEFAULT_MAP_ROOT / "data" / "map_space_revised_v2_saturation_features.csv"
DEFAULT_OUTPUT_NORMALIZED = DEFAULT_MAP_ROOT / "data" / "map_space_revised_v2_saturation_features_normalized.csv"
DEFAULT_NORM_PARAMS = DEFAULT_MAP_ROOT / "data" / "map_space_revised_v2_saturation_features_norm_params.csv"
DEFAULT_REPORT = DEFAULT_MAP_ROOT / "docs" / "map_space_revised_v2_saturation_features_report.md"
DEFAULT_EXCLUDED = DEFAULT_MAP_ROOT / "data" / "map_space_revised_v2_saturation_features_excluded.csv"
...
def extract_one_map(
    *,
    ...
    wkt_path_abs = (mrow.get("wkt_path") or "").strip()
    if roads_rel:
        roads = map_root / roads_rel
    elif wkt_path_abs:
        roads = Path(wkt_path_abs)
    elif wkt_rel:
    ...
    meta_abs = (mrow.get("metadata_path") or "").strip()
    if meta_abs and Path(meta_abs).is_file():
        meta_path = Path(meta_abs)
...
def main() -> None:
    ...
    parser.add_argument(
        "--geometry-only-normalized",
        action="store_true",
        help="Write normalized CSV without source_type one-hot columns (revised v2).",
    )
    ...
    if not args.no_normalize and included_rows:
        ...
        if args.geometry_only_normalized:
            for nr in norm_rows:
                nr.pop("source_type_osm", None)
                nr.pop("source_type_synthetic", None)
                nr.pop("source_type_trace_reference_synthetic", None)
