...
| **analysis/** | Extracción de features, correlación, métricas de salida, figuras y [dashboard interactivo](analysis/README.md). |
| **CHECKLIST_ESCENARIOS.md** | Checklist y diseño del corpus. |
...
Detalle de fases y opciones en [analysis/README.md](analysis/README.md); referencia de opciones .settings más abajo en este documento.
...
Para generar 50–60 escenarios variados y reducir correlación entre ellos, conviene variar de forma **sistemática** (pero no lineal) los siguientes ejes:
...
Combinando valores de forma **ortogonal** (p. ej. 4 duraciones × 5 seeds × 3 tamaños de red) se obtienen muchos escenarios; luego se puede filtrar o muestrear para quedarse con 50–60 y comprobar correlación (p. ej. análisis de correlación entre métricas o entre parámetros).
...
Con esta guía puedes definir de forma precisa cada aspecto del escenario y planificar el corpus de 50–60 escenarios para la validación de protocolos de enrutamiento.
...
- **Script principal:** `scenarios/analysis/run_analysis.py`, ejecutable por fases: `features` → `normalize` → `correlation` → `figures` → `output_metrics` → `outputs`. Ver `scenarios/analysis/README.md` para la lista completa de fases y opciones.
