## Escenario R2 — R2_VillagesTrails_ThreeClusters

### 1. Visión general

- **Scenario ID:** R2  
- **Nombre:** R2_VillagesTrails_ThreeClusters  
- **Familia:** Rural  
- **Fichero settings:** `corpus_v1/04_rural/R2_VillagesTrails_ThreeClusters.settings`

**Objetivo**

Tres clusters espaciales separados (aldeas); movilidad dentro de cada uno. Baseline ClusterMovement para comunidades rurales.
...
|---------|-------|------------|
| world_area | 25000000 | Área total de simulación (m^2) |
| aspect_ratio | 1 | min(width,height)/max(width,height) |
| N | 36 | Nodos totales |
| nrofHostGroups | 3 | Número de host groups |
| speed_mean | 0.6 | Velocidad media configurada (m/s) |
| wait_mean | 330 | Tiempo medio de pausa/espera (s) |
...
| clusterRange_mean | 350 | Radio medio de cluster si hay ClusterMovement |

### 3. Modelo de movilidad

Tres grupos ClusterMovement, 12 nodos cada uno, clusterRange 350 m.
